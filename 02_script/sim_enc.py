...
def gen_do_file( cur_time ):
   ...
   fo.write("vlog -L mtiAvm -L mtiOvm -L mtiUvm -L mtiUPF  +incdir+../00_src/rs_enc/+../03_tb/ -f ../00_src/rs_enc/src_enc.lst ../03_tb/harness_enc.sv\n")
   
   fo.write("vsim -sv_lib $UVM_DPI_HOME/uvm_dpi  -pli novas.dll -novopt work.harness_enc +SYS_DATE_TIME=" + cur_time + " +UVM_TESTNAME=$TEST_NAME +UVM_VERBOSITY=UVM_MEDIUM \n")
   
   fo.write("do ../02_script/wave_enc.do\n")
...
def gen_cfg_file( blk_n,m,n_s,r ):
   # 打开一个文件
   fo     = open("../03_tb/alg_rm/rs_enc_cfg.m", "w+")
   fo_svh = open("../03_tb/rs_enc_cfg.svh", "w+")
...
for m_key in r_dict.keys():
    ...
    for r_key in (r_dict[m_key]):
        ...
        for n_key in (n_dict[m_key]):
            ...
            os.system("matlab -wait -nodesktop -nosplash -r rs_en_test")
            os.chdir("../../")   #修改当前工作目录
            # print(os.getcwd())    #获取当前工作目录
            ...
            os.makedirs('11_log/DUT_enc')
            os.makedirs('11_log/RM_enc')
            ...
            os.system(vsim + " -c -do ../02_script/sim_enc.do")
